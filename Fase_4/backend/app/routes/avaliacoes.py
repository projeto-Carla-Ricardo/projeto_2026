...
@aval_bp.route('/<int:id>/validar', methods=['GET'])
@login_required
def validar(id):
    """Verifica completude da avaliação antes de finalizar."""
    aval = Avaliacao.query.filter_by(id=id, user_id=request.current_user.id).first()
    if not aval:
        return jsonify({'error': 'Avaliação não encontrada'}), 404

    todos_indicadores = Indicador.query.filter_by(obrigatorio=True).all()
    respondidos_ids = {r.indicador_id for r in Resposta.query.filter_by(avaliacao_id=id).all()}

    pendentes = []
    for ind in todos_indicadores:
        if ind.id not in respondidos_ids:
            pendentes.append({
                'indicador_id': ind.id,
                'codigo': ind.codigo,
                'pergunta': ind.pergunta,
                'componente_id': ind.componente_id
            })

    completo = len(pendentes) == 0
    return jsonify({
        'completo': completo,
        'total_obrigatorios': len(todos_indicadores),
        'respondidos': len(respondidos_ids),
        'pendentes': pendentes
    }), 200
...
def finalizar(id):
    ...
    if not aval:
        return jsonify({'error': 'Avaliação não encontrada'}), 404
    if aval.status in ('completa', 'concluida'):
